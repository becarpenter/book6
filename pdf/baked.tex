...
\item
  \hyperref[why-ipv6-is-so-complicated]{Why IPv6 is so complicated}
\end{itemize}
...
8 and 9 were used by proposals that were considered before
...
\hyperref[why-ipv6-is-so-complicated]{Next}
\hyperref[introduction-and-foreword]{Top}}{Previous Next Top}}\label{previous-next-top-7}

\pagebreak

\subsection{Why IPv6 is so
complicated}\label{why-ipv6-is-so-complicated}

There\textquotesingle s no question that IPv6 is more complicated than
IPv4, and people sometimes ask why that is. Surely it would have been
much simpler to just add an extra 32 bits to the IPv4 address, and
change nothing else? In fact, every year or two people propose
alternatives to IPv6 ("IPv8" is a generic name for such proposals, which
mainly involve 8-byte addresses) because they have asked themselves that
question. This note attempts to answer the question, and to show why
such proposals are a waste of everybody\textquotesingle s time,
especially for the people who propose them.
...
\item
  Just adding bits to the address isn\textquotesingle t as simple as it
  seems.
\item
  IPv4 was not the only network layer protocol in the world in 1994, and
  the others had good features that IPv4 lacked.
\item
...
1991 {[}\href{https://www.rfc-editor.org/info/rfc1287}{RFC1287}{]} and
led to an IESG report on future routing and addressing in late 1992
{[}\href{https://www.rfc-editor.org/info/rfc1380}{RFC1380}{]}. Concerns
about routing led to classless addressing and BGP4 routing; concerns
about address exhaustion led to agreement that a new version of IP was
needed. But scaling up the routing and addressing systems were not the
only concerns in RFC 1380 (see point 2 above):
...
{[}\href{https://www.rfc-editor.org/info/rfc1726}{RFC1726}{]}, and
explained in detail by
{[}\href{https://www.rfc-editor.org/info/rfc1752}{RFC1752}{]} and by the
book \emph{IPng: Internet Protocol Next Generation}, S. Bradner and A.
Mankin (editors), Addison-Wesley, 1995.

\subsubsection{Why adding bits isn\textquotesingle t
simple}\label{why-adding-bits-isnt-simple}
...
it\textquotesingle s a matter of mathematical and physical fact that to
expand the address size, you must change the protocol, and that means
...
Furthermore, you don\textquotesingle t want to split the Internet in
two, so you must design a method of interworking between the old version
and the new version. Annoyingly, you need to do that in a way that can
be done completely in machines that know about the new version, because
other machines don\textquotesingle t know anything at all about the new
version, by definition. So,
...
\tightlist
\item
  You need a coexistence technique so that updated systems, with the new
  protocol, can connect to old systems that know nothing of the new
  protocol.
\end{enumerate}
...
{[}\href{https://www.rfc-editor.org/info/rfc1671}{RFC1671}{]}, although
people still sometimes try to deny it.

To state this in graphical form, here\textquotesingle s a diagram,
showing who can talk to who, assuming the simplistic model of IPng with
64 bit addresses:
...
Protocol details, and the exact address length, don\textquotesingle t
matter. The XX cases can only work with protocol and address
translation. All the complexities (and that really means all) of
IPv4-IPng coexistence and transition are a result of this diagram, and
the details of IPng design do not change this fact of nature.
...
Incidentally, "IPv8" proponents often ask why IPv6
didn\textquotesingle t simply stick some extra bits on the front of IPv4
addresses, instead of inventing a whole new format. Actually, we tried
that: the "IPv4-Compatible IPv6 address" format was defined in
{[}\href{https://www.rfc-editor.org/info/rfc3513}{RFC3513}{]} but
deprecated by
{[}\href{https://www.rfc-editor.org/info/rfc4291}{RFC4291}{]} because it
turned out to be of no practical use for coexistence or transition. The
related "IPv4-Mapped IPv6 address" format is still valid and has a role
in the POSIX socket API. Mappings of this kind also figured in the
moderately successful coexistence technologies known as 6to4
{[}\href{https://www.rfc-editor.org/info/rfc3056}{RFC3056},
\href{https://www.rfc-editor.org/info/rfc3068}{RFC3068}{]} and Teredo
{[}\href{https://www.rfc-editor.org/info/rfc4380}{RFC4380}{]}, which
have now been overtaken by events.

Finally, it\textquotesingle s worth remembering that IPv4 is itself no
longer simple. Compared with 1994, we now have a whole lot of
complications: subscriber-side NAT, carrier-grade NAT, firewalls, IPsec,
VPNs, Differentiated Services, link-local addresses, content
distribution networks, etc.

\subsubsection{The protocol zoo}\label{the-protocol-zoo}
...
\emph{better than OSI}.

\subsubsection{Did the IPv6 designers go
mad?}\label{did-the-ipv6-designers-go-mad}

That might be going a bit far, so the question should probably be: Did
the IPv6 design suffer from Second System Syndrome,
\href{https://en.wikipedia.org/wiki/Second-system_effect}{"the tendency
for a successful first system (often small and relatively elegant) to be
followed by a second system that becomes over-engineered or bloated"}?
Did we add stuff for its own sake?

The objective answer is "not much." First of all, IPv6 really is a
conservative design - it doesn\textquotesingle t change the basic IP
model of connectionless packet switching with topological addresses. We
added the flow label (harmless if unused, as it was for many years). We
tweaked the fragmentation mechanism. We replaced IPv4 "options" with
IPv6 "extension headers". (Just as IPv4 options are largely unused, so
are IPv6 extension headers, except within limited domains.) Most
important, we \emph{added} Stateless Address Autoconfiguration (SLAAC)
and the closely linked router advertisement (RA) mechanism, and the
linked idea of an interface identifier (IID) as part of the address.
SLAAC was inspired by DECNET, Netware and Appletalk, none of which
required manual address configuration. SLAAC is partly redundant with
DHCPv6, and the reason for that is that DHCP was new and unproven when
SLAAC and RA were designed; DHCPv6 was actually a retrofit.
...
decision to make IPv6 \emph{require} support for IP Security (IPsec),
which was an immature technology at the time. This was a definite brake
on IPv6 deployment until it was dropped after some years.

\subsubsection{Do not make things worse}\label{do-not-make-things-worse}

It\textquotesingle s worth adding that some of the "IPv8" proposals over
the years have included ideas that would make things worse rather than
better - things like geographic addressing, address prefixes based on
Autonomous System numbers, addresses with semantics encoded in their
bits, and the list goes on. All these things would break Internet
routing, make site renumbering even harder, risk running out of
addresses yet again, simplify pervasive surveillance, or cause other
forms of operational harm. (To take one example, although geographic
addressing has some obvious advantages when geolocation is wanted, it is
incompatible with the way Internet interdomain routing works.)

\subsubsection{It would take 25 years
anyway}\label{it-would-take-25-years-anyway}
...
\subsubsection{Postscriptum}\label{postscriptum}
...
{[}\href{https://www.rfc-editor.org/info/rfc8507}{RFC8507}{]}.

Paul Francis, 1992, "The \textquotesingle P\textquotesingle{} Internet
Protocol" (PIP), an IPng candidate
{[}\href{https://www.rfc-editor.org/info/rfc1621}{RFC1621},
\href{https://www.rfc-editor.org/info/rfc1622}{RFC1622}{]} was
\emph{officially} IPv8 for a while. It had variable length addresses.

Bob Hinden and Steve Deering 1993/4, "Simple Internet Protocol Plus"
(SIPP), an IPng candidate
{[}\href{https://www.rfc-editor.org/info/rfc1710}{RFC1710}{]}, had
8-byte addresses. It inherited SIP\textquotesingle s use of version
number 6. At the end of the IPng decision process, it mutated to 16-byte
addresses and was the immediate precursor of IPv6.

{[}\href{https://datatracker.ietf.org/doc/draft-carpenter-aeiou/}{draft-carpenter-aeiou}{]}
(1994)
...
\href{https://afnog.org/archives/archives/msg01304.html}{AFNOG}:
...
and some
\href{https://web.archive.org/web/20000412004245/http://www.unir.com/images/headers.gif}{archived
diagrams}.

{[}\href{https://datatracker.ietf.org/doc/draft-terrell-ip-spec-ipv7-ipv8-addr-cls/}{draft-terrell-ip-spec-ipv7-ipv8-addr-cls}{]},
{[}\href{https://datatracker.ietf.org/doc/draft-terrell-logic-analy-bin-ip-spec-ipv7-ipv8/}{draft-terrell-logic-analy-bin-ip-spec-ipv7-ipv8}{]}
(1999)

{[}\href{https://datatracker.ietf.org/doc/draft-shyam-real-ip-framework/}{draft-shyam-real-ip-framework}{]}
(2014)

{[}\href{https://datatracker.ietf.org/doc/draft-omar-ipv10/}{draft-omar-ipv10}{]}
(2016)

{[}\href{https://datatracker.ietf.org/doc/draft-sambana-irtf-internet-protocol-sixteen/}{draft-sambana-irtf-internet-protocol-sixteen}{]}
(2022)

{[}\href{https://datatracker.ietf.org/doc/draft-thain-ipv8/}{draft-thain-ipv8}{]}
(2026)

{[}\href{https://datatracker.ietf.org/doc/draft-hause-asip/}{draft-hause-asip}{]}
(2026)

{[}\href{https://datatracker.ietf.org/doc/draft-subbiah-ipv7/}{draft-subbiah-ipv7}{]}
(2026)

Here\textquotesingle s an
\href{https://www.ip.network/blog/what-is-ipv8-protocol}{interesting
blog}.

\subsubsection{\texorpdfstring{\hyperref[why-version-6]{Previous}
\hyperref[ipv6-basic-technology]{Next}
\hyperref[introduction-and-foreword]{Top}}{Previous Next Top}}\label{previous-next-top-8}
...
\href{https://www.rfc-editor.org/info/rfc5952}{the recommendation by RFC
5952 today} is to use lower-case letters for hexadecimal. Thus a basic
...
in (Web) URLs
{[}\href{https://www.rfc-editor.org/info/rfc3986}{RFC3986}{]}, so IPv6
...
\subsubsection{Literal addresses in web browsers and
URLs}\label{literal-addresses-in-web-browsers-and-urls}
...
{[}\href{https://www.rfc-editor.org/info/rfc3986}{RFC3986}{]}, e.g.:
...
\href{https://www.rfc-editor.org/info/rfc9119}{RFC9119}{]} but it
...
draft}. More generally,
\href{https://www.rfc-editor.org/info/rfc9898}{RFC 9898} summarizes
...
\href{https://www.rfc-editor.org/info/rfc9685}{RFC 9685}. These
...
DHCPv6 is defined by \href{https://www.rfc-editor.org/info/rfc9915}{RFC
9915}. It is conceptually similar to DHCP for IPv4, but different in
...
but now covered by
\href{https://www.rfc-editor.org/rfc/rfc9915.html\#section-6.3}{Section
6.3 of RFC9915}. This process is known as DHCPv6-PD (for "prefix
...
\href{https://www.rfc-editor.org/info/rfc9915}{RFC 9915} directly. Be
...
\hyperref[ipv6-basic-technology]{Top}}{Previous Next Top}}\label{previous-next-top-19}
...
{[}\hyperref[obsolete-techniques]{Obsolete techniques}{]}). Conversely,
IPv4 can be carried as a service over IPV6-only infrastructure with
BGP-4 support
{[}\href{https://www.rfc-editor.org/info/rfc8950}{RFC8950}{]}, as
mentioned in
\href{https://github.com/becarpenter/book6/blob/main/02.\%20IPv6\%20Basic\%20Technology/Routing.md}{2.
Routing}.
...
\hyperref[tunnels]{above}) is sufficient, but typically translation
between IPv6 and IPv4 is unavoidable. Especially, when providing IPv4 as
a Service (IPv4aaS), a typical scenario will be:
...
\item
  ...
  server.
\item
  Translate the traffic of local IPv6 clients to and from remote
  IPv4-only servers, often using a centralized NAT64 device.
\item
  Encapsulate literal IPv4 address requests into IPv6 on the client then
  decapsulate and translate it on the NAT64 to access the IPv4 server.
\end{enumerate}
...
\item
  ...
  {[}\href{https://www.rfc-editor.org/info/rfc7755}{RFC7755},
  \href{https://www.rfc-editor.org/info/rfc7756}{RFC 7756}{]}
\item
  ...
  \tightlist
  \item
    \href{https://www.rfc-editor.org/info/rfc7757}{RFC 7757} (also
    \href{https://www.rfc-editor.org/info/std103}{STD 103}) defines an
    explicit address mapping algorithm.
  \item
    \href{https://www.rfc-editor.org/info/rfc6052}{RFC 6052} describes
    algorithmic translation (mapping) between an IPv6 address and a
    corresponding IPv4 address. A Well-Known Prefix (WKP), 64:ff9b::/96,
    is used for translation.
  \item
    \href{https://www.rfc-editor.org/info/rfc8215}{RFC 8215} adds
    64:ff9b:1::/48 as the Network-Specific Prefix (NSP) for translation
    between an IPv6 address and a local (private) IPv4 address.
  \item
  ...
  \item
    ...
    supports DNS extensions for clients of stateful NAT64. It is
    problematic in some deployments because of incompatibility with DNS
    security (DNSSEC). DNS64 synthesizes AAAA records from A records,
  ...
  \item
    ...
    (or \href{https://www.rfc-editor.org/info/rfc7050}{RFC 7050} plus
    \href{https://www.rfc-editor.org/info/rfc8880}{RFC 8880}) are
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={User devices connected to Internet via IPv6 infrastructure}]{vasilenko-IPv4aaS.png}}
...
\item
  In the context of NAT64, DNS64
  {[}\href{https://www.rfc-editor.org/info/rfc6147}{RFC 6147}{]} is
  incompatible with DNSSEC, and
  \href{https://www.rfc-editor.org/info/rfc8781}{RFC 8781} is now
  preferable to \href{https://www.rfc-editor.org/info/rfc7050}{RFC 7050}
  plus \href{https://www.rfc-editor.org/info/rfc8880}{RFC 8880}.
\item
  "Bump-In-the-Stack" (BIS)
  {[}\href{https://www.rfc-editor.org/info/rfc2767}{RFC 2767}{]},
  "Bump-in-the-API" (BIA)
  {[}\href{https://www.rfc-editor.org/info/rfc3338}{RFC 3338}{]} and
  "Bump-in-the-Host" (BIH)
  {[}\href{https://www.rfc-editor.org/info/rfc6535}{RFC 6535}{]} are no
  longer useful, since
  \hyperref[translation-and-ipv4-as-a-service]{464XLAT} provides a
  better solution.
\end{itemize}
...
\hyperref[coexistence-with-legacy-ipv4]{Top}}{Previous Next Top}}\label{previous-next-top-23}
...
\hyperref[security]{Top}}{Previous Next Top}}\label{previous-next-top-25}
...
prefix delegation}
{[}\href{https://www.rfc-editor.org/info/rfc9915}{RFC9915}{]}. Instead
...
{[}\href{https://www.rfc-editor.org/info/rfc9915}{RFC9915}{]}, whereas
...
\hyperref[network-design]{Top}}{Previous Next Top}}\label{previous-next-top-26}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Routers and routing clouds as described above}]{multiPrefix.png}}
...
\hyperref[management-and-operations]{Top}}{Previous Next Top}}\label{previous-next-top-35}
...
You can check your own status immediately
\href{https://test-ipv6.com/}{here}.

You can check any web site you want
\href{https://webres6.dev.sap/}{here}.
...
spot in China where adoption is significant. The Google statistic first
touched 50\% in March 2026.
...
summarized by
\href{https://radar.cloudflare.com/adoption-and-usage}{Cloudflare
radar}.
...
\hyperref[deployment-status]{Top}}{Previous Next Top}}\label{previous-next-top-38}
...
\hyperref[troubleshooting]{Top}}{Previous Next Top}}\label{previous-next-top-39}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagram of RFC status}]{rfc-diagram.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagram of I-D status}]{id-diagram.png}}
...
\subsubsection{Standards Track (272
RFCs)}\label{standards-track-272-rfcs}
...
\item
  \href{https://www.rfc-editor.org/info/rfc9928}{RFC 9928}: DHCPv4 over
...
\subsubsection{Informational (193 RFCs)}\label{informational-193-rfcs}
...
\item
  \href{https://www.rfc-editor.org/info/rfc7051}{RFC 7051}: Analysis of
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Start here, end here}]{example1.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Disk feeding tape}]{diag.png}}
...
\hyperref[chapter-template]{Top}}{Previous Next Top}}\label{previous-next-top-42}
...
\hyperref[why-ipv6-is-so-complicated]{6to4 ¶}
\hyperref[obsolete-techniques]{¶}
...
\hyperref[why-ipv6-is-so-complicated]{¶} \hyperref[why-version-6]{¶}
...
\hyperref[why-ipv6-is-so-complicated]{BGP ¶} \hyperref[addresses]{¶}
\hyperref[routing]{¶} \hyperref[tunnels]{¶} \hyperref[filtering]{¶}
\hyperref[multi-prefix-operation]{¶} \hyperref[multihoming]{¶}
\hyperref[routing-operation]{¶} \hyperref[tools]{¶}
...
\hyperref[translation-and-ipv4-as-a-service]{CLAT ¶}

\hyperref[why-ipv6-is-so-complicated]{coexistence ¶}
\hyperref[coexistence-with-legacy-ipv4]{¶}
...
\hyperref[why-ipv6-is-so-complicated]{DHCP ¶}
...
\hyperref[why-ipv6-is-so-complicated]{differentiated services ¶}
...
\hyperref[why-ipv6-is-so-complicated]{¶} \hyperref[addresses]{¶}
\hyperref[auto-configuration]{¶} \hyperref[dns]{¶}
\hyperref[managed-configuration]{¶} \hyperref[dual-stack-scenarios]{¶}
\hyperref[translation-and-ipv4-as-a-service]{¶}
...
\hyperref[why-ipv6-is-so-complicated]{¶} \hyperref[routing]{¶}
\hyperref[transport-protocols]{¶}
\hyperref[coexistence-with-legacy-ipv4]{¶}
\hyperref[dual-stack-scenarios]{¶}
\hyperref[translation-and-ipv4-as-a-service]{¶} \hyperref[tunnels]{¶}
...
\hyperref[why-ipv6-is-so-complicated]{firewall ¶}
...
\hyperref[why-ipv6-is-so-complicated]{flow label ¶}
\hyperref[packet-format]{¶} \hyperref[routing]{¶}
...
\hyperref[why-ipv6-is-so-complicated]{IID ¶} \hyperref[addresses]{¶}
\hyperref[ipv6-primary-differences-from-ipv4]{¶}
\hyperref[translation-and-ipv4-as-a-service]{¶}
\hyperref[layer-2-considerations]{¶}
\hyperref[multi-prefix-operation]{¶} \hyperref[multihoming]{¶}
...
\hyperref[why-ipv6-is-so-complicated]{IPsec ¶}
...
\hyperref[why-ipv6-is-so-complicated]{link-local ¶}
\hyperref[address-resolution]{¶} \hyperref[addresses]{¶}
\hyperref[auto-configuration]{¶} \hyperref[dns]{¶}
\hyperref[source-and-destination-address-selection]{¶}
...
\hyperref[why-ipv6-is-so-complicated]{NAT ¶}
\hyperref[transport-protocols]{¶} \hyperref[dual-stack-scenarios]{¶}
\hyperref[obsolete-techniques]{¶}
\hyperref[translation-and-ipv4-as-a-service]{¶} \hyperref[tunnels]{¶}
...
\hyperref[translation-and-ipv4-as-a-service]{NSP ¶}
...
\hyperref[translation-and-ipv4-as-a-service]{PLAT ¶}
...
\hyperref[why-ipv6-is-so-complicated]{prefix ¶} \hyperref[addresses]{¶}
\hyperref[auto-configuration]{¶} \hyperref[managed-configuration]{¶}
\hyperref[routing]{¶}
\hyperref[source-and-destination-address-selection]{¶}
\hyperref[dual-stack-scenarios]{¶}
...
\hyperref[why-ipv6-is-so-complicated]{RA messages ¶}
\hyperref[address-resolution]{¶} \hyperref[auto-configuration]{¶}
\hyperref[managed-configuration]{¶} \hyperref[routing]{¶}
\hyperref[prefix-per-host]{¶} \hyperref[multi-prefix-operation]{¶}
\hyperref[advanced-troubleshooting]{¶} \hyperref[tools]{¶}
\hyperref[obsolete-features-in-ipv6]{¶}
...
\hyperref[why-ipv6-is-so-complicated]{route ¶}
...
\hyperref[why-ipv6-is-so-complicated]{security ¶} \hyperref[dns]{¶}
\hyperref[extension-headers-and-options]{¶}
\hyperref[managed-configuration]{¶} \hyperref[packet-format]{¶}
\hyperref[source-and-destination-address-selection]{¶}
\hyperref[dual-stack-scenarios]{¶}
...
\hyperref[why-ipv6-is-so-complicated]{SIP ¶} \hyperref[why-version-6]{¶}
\hyperref[transport-protocols]{¶} \hyperref[dual-stack-scenarios]{¶}
\hyperref[translation-and-ipv4-as-a-service]{¶}
\hyperref[multihoming]{¶}

\hyperref[why-ipv6-is-so-complicated]{SLAAC ¶}
\hyperref[auto-configuration]{¶} \hyperref[managed-configuration]{¶}
\hyperref[routing]{¶} \hyperref[ipv6-primary-differences-from-ipv4]{¶}
\hyperref[security]{¶} \hyperref[filtering]{¶}
\hyperref[layer-2-considerations]{¶}
\hyperref[address-and-prefix-management]{¶}
\hyperref[multi-prefix-operation]{¶}
...
\hyperref[why-ipv6-is-so-complicated]{Teredo ¶}
\hyperref[obsolete-techniques]{¶}
...
\hyperref[translation-and-ipv4-as-a-service]{WKP ¶}
...
\hyperref[why-ipv6-is-so-complicated]{RFC1287 ¶}

\hyperref[why-ipv6-is-so-complicated]{RFC1380 ¶}

\hyperref[why-version-6]{RFC1475 ¶}

\hyperref[why-version-6]{RFC1606 ¶}

\hyperref[why-ipv6-is-so-complicated]{RFC1621 ¶}

\hyperref[why-ipv6-is-so-complicated]{RFC1622 ¶}

\hyperref[why-ipv6-is-so-complicated]{RFC1671 ¶}

\hyperref[why-version-6]{RFC1700 ¶}

\hyperref[why-ipv6-is-so-complicated]{RFC1710 ¶}

\hyperref[why-ipv6-is-so-complicated]{RFC1726 ¶}

\hyperref[why-ipv6-is-so-complicated]{RFC1752 ¶}
...
\hyperref[obsolete-techniques]{RFC2767 ¶}

\hyperref[obsolete-techniques]{RFC3053 ¶}

\hyperref[why-ipv6-is-so-complicated]{RFC3056 ¶}
\hyperref[obsolete-techniques]{¶}

\hyperref[why-ipv6-is-so-complicated]{RFC3068 ¶}
\hyperref[obsolete-techniques]{¶}
...
\hyperref[obsolete-techniques]{RFC3338 ¶}

\hyperref[source-and-destination-address-selection]{RFC3484 ¶}

\hyperref[why-ipv6-is-so-complicated]{RFC3513 ¶}
\hyperref[obsolete-features-in-ipv6]{¶} \hyperref[markdown-usage]{¶}
...
\hyperref[why-ipv6-is-so-complicated]{RFC4291 ¶}
\hyperref[address-resolution]{¶} \hyperref[addresses]{¶}
\hyperref[filtering]{¶} \hyperref[obsolete-features-in-ipv6]{¶}
\hyperref[markdown-usage]{¶}
...
\hyperref[why-ipv6-is-so-complicated]{RFC4380 ¶}
\hyperref[obsolete-techniques]{¶}
...
\hyperref[translation-and-ipv4-as-a-service]{RFC6052 ¶}
...
\hyperref[obsolete-techniques]{RFC6147 ¶}
\hyperref[translation-and-ipv4-as-a-service]{¶}
...
\hyperref[obsolete-techniques]{RFC6535 ¶}
...
\hyperref[translation-and-ipv4-as-a-service]{RFC7755 ¶}

\hyperref[translation-and-ipv4-as-a-service]{RFC7756 ¶}

\hyperref[translation-and-ipv4-as-a-service]{RFC7757 ¶}
...
\hyperref[translation-and-ipv4-as-a-service]{RFC8215 ¶}
...
\hyperref[why-ipv6-is-so-complicated]{RFC8507 ¶}
...
\hyperref[obsolete-techniques]{RFC8781 ¶}
\hyperref[translation-and-ipv4-as-a-service]{¶}

\hyperref[traffic-class-and-flow-label]{RFC8837 ¶}

\hyperref[obsolete-techniques]{RFC8880 ¶}
\hyperref[translation-and-ipv4-as-a-service]{¶}
...
\hyperref[routing]{RFC8950 ¶} \hyperref[tunnels]{¶}
...
\hyperref[address-resolution]{RFC9898 ¶}

\hyperref[managed-configuration]{RFC9915 ¶}
\hyperref[address-planning]{¶} \hyperref[prefix-per-host]{¶}

\hyperref[translation-and-ipv4-as-a-service]{STD103 ¶}
