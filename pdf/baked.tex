...
A little tip: For some reason, GitHub doesn\textquotesingle t support
automatically opening a link in a new browser tab or window, so clicking
on links will always take you to a new page. To avoid this, with most
browsers you can use CTRL+click (on Windows and Linux) or CMD+click (on
MacOS).
...
\item
  ...
  \href{https://www.rfc-editor.org/info/rfc2474}{differentiated
  services} code point followed by two
...
annoying in one particular case - when including an IPv6 address in a
(Web) URL, where a colon has another meaning and works as a separator
between address and port. That\textquotesingle s why IPv6 addresses in
...
the subnet prefix is \texttt{2001:db8:4006:80b/64} or
\texttt{fd63:45eb:dc14:1/64}. This is the normal setting in IPv6:
subnets have 64 bit prefixes and 64 bit IIDs.
\hyperref[auto-configuration]{Automatic address configuration} depends
on this fixed boundary. Links that don\textquotesingle t use automatic
address configuration are not bound by the /64 rule, but a lot of
software and configurations rely on it.
...
enable this, multiprotocol BGP-4 has provision to advertise IPv4
...
so was not a design goal.} The only solution was a new version number,
and in 1994, the next free number was 6. (IPv5 was a failed experiment.)
...
that it can be GUA to GUA, and this is a definite valid, supportable,
and definitely production deployed architecture.
...
model, suck as an SD-WAN deployment, a GUA or ULA prefix may be
